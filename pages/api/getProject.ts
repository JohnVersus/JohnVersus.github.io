...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const description = (dom.window.document.head.querySelector('meta[name="description"]') as unknown as HTMLMetaElement)
    ?.content;
  // OG Data
  ...
  const twitterUrl = (dom.window.document.head.querySelector('meta[name="twitter:site"]') as unknown as HTMLMetaElement)
    ?.content;
  const twitterTitle = (
